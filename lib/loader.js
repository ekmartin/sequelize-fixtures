...
Loader.prototype.prepFixtureData = function(data, Model) {
    ...
    Object.keys(data).forEach(function(key) {
        ...
        if (assoc) {
            if (assoc.associationType === 'BelongsTo') {
            ...
            } else if (['BelongsToMany', 'hasMany'].indexOf(assoc.associationType) >= 0) {
                if (Array.isArray(val)) {
                    ...
                    throw new Error('HasMany/BelongsToMany associations must be arrays of where clauses');
                }
            } else {
                throw new Error('Only BelongsTo, BelongsToMany and HasMany associations are supported');
            }
